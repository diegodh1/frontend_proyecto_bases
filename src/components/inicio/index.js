...
const Inicio = () => {
  const { message, empleado } = useSelector(state => ({
  ...
  const ingresar = () => {
    if (!Number(cedula) || contrasenha.length <= 6) {
      ...
      fetch('http://localhost:4000/login_empleado', {
        ...
        .then(response => {
          response.message !== "Ingreso realizado" ? dispatch(error_login_empleado(response)) : dispatch(success_login_empleado(response));
        ...
        <div className={classes.paper}>
          ...
          <form className={classes.form} noValidate>
            ...
            <Button
              ...
              variant="contained"
              onClick={() => ingresar()}
              style={{ background: '#4eb84e', MozBorderRadius: 40, color: '#383736', fontSize: 20, font: 'bold' }}
